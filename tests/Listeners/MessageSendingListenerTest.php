...
it('runs the middleware pipelines and returns if the message should be sent', function (bool $shouldSendMessage, bool $shouldLog) {
    ...
    Config::set('mail-allowlist.sending.middleware_enabled', true);
    Config::set('mail-allowlist.sending.middleware', $middleware = ['::middleware::']);
...
it('does not run the middleware if disabled', function () {
    Config::set('mail-allowlist.enabled', true);
    Config::set('mail-allowlist.sending.middleware_enabled', false);
    Config::set('mail-allowlist.sending.middleware', ['::middleware::']);
